...
static auto lib_subscribe_callback(const z_sample_t *sample, void *arg) //
  ...
  const auto var = flunder_variable_t{
      ...
      std::string{reinterpret_cast<const char *>(sample->payload.start),
                  sample->payload.len},
      to_string(sample->encoding.prefix,
  ...
  std::visit(overload{// call callback without userdata
...
auto to_string(z_encoding_prefix_t prefix, std::string_view suffix) //
  ...
  return (it != strings.cend())
             ? std::string{it->second.data()} + std::string{suffix}
             : std::string{suffix};
}
...
flunder_client_t::flunder_client_t()
    : _mem_storages{}, _z_session{}, _subscriptions{} {}
...
auto flunder_client_t::connect(std::string_view host, int port) //
  ...
  const auto len =
      std::snprintf(nullptr, 0, "[\"tcp/%s:%d\"]", host.data(), port);
  auto remote = std::string(len, '0');
  std::snprintf(remote.data(), remote.length() + 1, "[\"tcp/%s:%d\"]",
                host.data(), port);

  auto config = z_config_default();
  zc_config_insert_json(z_config_loan(&config), Z_CONFIG_CONNECT_KEY,
                        remote.c_str());
  zc_config_insert_json(z_config_loan(&config), Z_CONFIG_MODE_KEY,
                        R"#("client")#");
  zc_config_insert_json(z_config_loan(&config), Z_CONFIG_MULTICAST_SCOUTING_KEY,
                        "false");
...
auto flunder_client_t::is_connected() const noexcept //
  ...
  const auto session_initialized =
      !std::equal(reinterpret_cast<const char *>(&_z_session),
                  reinterpret_cast<const char *>(&_z_session) +
                      sizeof(decltype(_z_session)),
...
auto flunder_client_t::publish_bool(std::string_view topic,
                                    const std::string &value) const //
    -> int {
  return publish(topic, z_encoding(Z_ENCODING_PREFIX_APP_CUSTOM, "bool"),
                 value);
}

auto flunder_client_t::publish_int(std::string_view topic, size_t size,
                                   bool is_signed,
                                   const std::string &value) const //
    -> int {
  ...
  return publish(
      topic, z_encoding(Z_ENCODING_PREFIX_APP_INTEGER, suffix.c_str()), value);
}

auto flunder_client_t::publish_float(std::string_view topic, size_t size,
                                     const std::string &value) const //
    -> int {
  const auto size_str = stringify("+", size * 8);
  return publish(
      topic, z_encoding(Z_ENCODING_PREFIX_APP_FLOAT, size_str.c_str()), value);
}

auto flunder_client_t::publish_string(std::string_view topic,
                                      const std::string &value) const //
    -> int {
  return publish(topic, z_encoding(Z_ENCODING_PREFIX_TEXT_PLAIN, nullptr),
                 value);
}

auto flunder_client_t::publish_raw(std::string_view topic, const void *payload,
                                   size_t payloadlen) const //
    -> int {
  return publish(
      topic, z_encoding(Z_ENCODING_PREFIX_APP_OCTET_STREAM, nullptr),
      std::string{reinterpret_cast<const char *>(payload), payloadlen});
}

auto flunder_client_t::publish_custom(std::string_view topic,
                                      const void *payload, size_t payloadlen,
                                      std::string_view encoding) const //
    -> int {
  const auto [prefix, suffix] = encoding_from_string(encoding);
  return publish(
      topic, z_encoding(prefix, suffix.data()),
      std::string{reinterpret_cast<const char *>(payload), payloadlen});
}

auto flunder_client_t::publish(std::string_view topic, z_encoding_t encoding,
                               const std::string &value) const //
    -> int {
  auto options = z_put_options_default();
  options.encoding = encoding;
  options.congestion_control =
      z_congestion_control_t::Z_CONGESTION_CONTROL_BLOCK;

  const auto keyexpr =
      cxx20::starts_with(topic, '/') ? topic.data() + 1 : topic.data();

  const auto res = z_put(z_session_loan(&_z_session), z_keyexpr(keyexpr),
...
auto flunder_client_t::subscribe(flunder::flunder_client_t *client,
  ...
  auto res = _subscriptions.emplace(
      keyexpr, subscribe_ctx_t{client, {}, cbk, userp, false});
  if (!res.second) {
    return -1;
  }
  ...
  auto closure = z_owned_closure_sample_t{
      .context = &ctx, .call = lib_subscribe_callback, .drop = nullptr};
  ctx._sub =
      z_declare_subscriber(z_session_loan(&_z_session), z_keyexpr(keyexpr),
...
auto flunder_client_t::add_mem_storage(std::string name,
                                       std::string_view topic) //
    -> int {
  ...
  const auto res =
      cpr::Put(url, cpr::Header{{"content-type", "application/json"}},
...
auto flunder_client_t::get(std::string_view topic) const //
  ...
  auto keyexpr = z_keyexpr(cxx20::starts_with(topic, '/') ? topic.data() + 1
                                                          : topic.data());
  if (!z_keyexpr_is_initialized(&keyexpr)) {
    return {-1, vars};
  }

  z_get(z_session_loan(&_z_session), keyexpr, "", z_move(reply_channel.send),
        &options);

  z_owned_reply_t reply = z_reply_null();
  for (z_reply_channel_closure_call(&reply_channel.recv, &reply);
       z_reply_check(&reply);
...
auto flunder_client_t::erase(std::string_view topic) //
  ...
  const auto res =
      z_delete(z_session_loan(&_z_session), z_keyexpr(keyexpr), &options);
...
auto ntp64_to_unix_time(std::uint64_t ntp_time) //
  ...
  const auto unix_time = static_cast<uint64_t>(
      (seconds + (fractions / std::numeric_limits<std::uint32_t>::max())) *
      1'000'000'000);
