...
template <typename T> auto topic(T);

#define TOPIC_BASE "/flecs/flunder/test/"
#define DEF_TOPIC(type)                                                        \
  template <> auto topic<type>(type) { return TOPIC_BASE #type; }
#define DEF_TOPIC_EX(type, suffix)                                             \
  template <> auto topic<type>(type) { return TOPIC_BASE #suffix; }
...
template <typename T> constexpr auto encoding(T);

template <typename T> constexpr auto is_signed(T) {
...
#define DEF_APP_ENCODING(type)                                                 \
  template <> constexpr auto encoding<type>(type) {                            \
    return "application/" #type;                                               \
  }
#define DEF_INT_ENCODING(type)                                                 \
  template <> auto encoding<type>(type) {                                      \
...
#define DEF_CUSTOM_ENCODING(type, enc)                                         \
  template <> auto encoding<type>(type) { return enc; }
...
TEST(flunder, pub_sub) {
  ...
  auto res = client_1.subscribe(topic(nullptr), flunder_cbk_userp<std::int32_t>,
                                reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);

  res = client_1.subscribe(topic(std::string{}), flunder_cbk_userp<std::string>,
                           reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);

  res = client_1.subscribe(topic(std::string_view{}),
                           flunder_cbk_userp<std::string_view>,
                           reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);

  res =
      client_1.subscribe(topic(std::int32_t{}), flunder_cbk_userp<std::int32_t>,
                         reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);

  res = client_1.subscribe(topic(bool{}), flunder_cbk_userp<bool>,
                           reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);

  res = client_1.subscribe(topic(float{}), flunder_cbk_userp<float>,
                           reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);

  res = client_1.subscribe(topic(double{}), flunder_cbk_userp<double>,
                           reinterpret_cast<const void *>(&client_1));
  ASSERT_EQ(res, 0);
  ...
  ASSERT_EQ(res, 0);
  usleep(250);
  res = client_2.publish(topic((void *)(nullptr)),
                         reinterpret_cast<void *>(0x0), 0);
  ASSERT_EQ(res, 0);
  usleep(250);
  res = client_2.publish(topic(custom_t{}), "Hello, FLECS!", 13,
                         encoding(custom_t{}));
  ASSERT_EQ(res, 0);
...
TEST(flunder, c) {
  ...
  res = flunder_subscribe_userp(client, topic(bool{}),
                                flunder_cbk_c_userp<bool>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::int8_t{}),
                                flunder_cbk_c_userp<std::int8_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::int16_t{}),
                                flunder_cbk_c_userp<std::int16_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::int32_t{}),
                                flunder_cbk_c_userp<std::int32_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::int64_t{}),
                                flunder_cbk_c_userp<std::int64_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::uint8_t{}),
                                flunder_cbk_c_userp<std::uint8_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::uint16_t{}),
                                flunder_cbk_c_userp<std::uint16_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::uint32_t{}),
                                flunder_cbk_c_userp<std::uint32_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(std::uint64_t{}),
                                flunder_cbk_c_userp<std::uint64_t>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(float{}),
                                flunder_cbk_c_userp<float>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic(double{}),
                                flunder_cbk_c_userp<double>, client);
  ASSERT_EQ(res, 0);
  res = flunder_subscribe_userp(client, topic((const char *)(nullptr)),
                                flunder_cbk_c_userp<const char *>, client);
  ASSERT_EQ(res, 0);
  ...
  usleep(250);
  res =
      flunder_publish_int16(client, topic(std::int16_t{}), val(std::int16_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  res =
      flunder_publish_int32(client, topic(std::int32_t{}), val(std::int32_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  res =
      flunder_publish_int64(client, topic(std::int64_t{}), val(std::int64_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  res =
      flunder_publish_uint8(client, topic(std::uint8_t{}), val(std::uint8_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  res = flunder_publish_uint16(client, topic(std::uint16_t{}),
                               val(std::uint16_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  res = flunder_publish_uint32(client, topic(std::uint32_t{}),
                               val(std::uint32_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  res = flunder_publish_uint64(client, topic(std::uint64_t{}),
                               val(std::uint64_t{}));
  ASSERT_EQ(res, 0);
  usleep(250);
  ...
  ASSERT_EQ(res, 0);
  usleep(250);
  res = flunder_publish_string(client, topic((const char *)(nullptr)),
  ...
  ASSERT_EQ(res, 0);
  usleep(250);
  res = flunder_publish_custom(client, topic(custom_t{}), nullptr, 0,
                               encoding(custom_t{}));
  ASSERT_EQ(res, 0);
